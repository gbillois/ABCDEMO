...
/* ============================================================================
 * runtime.js — browser shim layer for html2pptx1.js
 *
 * html2pptx1.js is a Node module. The single biggest piece of it
 * (extractSlideData, ~3300 lines) already runs INSIDE a browser via Playwright's
 * page.evaluate(), so it is pure DOM code. The only Node-specific dependencies
 * are a thin outer layer: playwright, sharp, fs, path, http/https, process,
 * Buffer. This file provides browser-native equivalents so the engine runs
 * unmodified in the page:
 *
 *   - playwright  -> a fake "browser/page" backed by a hidden same-origin <iframe>.
 *                    page.evaluate() compiles the engine's callback inside the
 *                    iframe realm (so `document`/`getComputedStyle` resolve to the
 *                    slide being measured) and returns a structured-cloned result.
 *   - sharp       -> a chainable canvas pipeline (resize / extract / composite).
 *   - fs/path     -> tiny in-memory stubs; "temp files" become a path->dataURL Map
 *                    (__TMPFS__) that the slide.addImage wrapper resolves to data.
 *   - http/https  -> fetch() based get() for image downloads (CORS permitting).
 *   - Buffer      -> Uint8Array-backed minimal Buffer.
 *
 * Everything here is defined BEFORE the engine source executes.
 * ==========================================================================*/

// path -> dataURL registry standing in for the engine's /tmp/*.png files.
const __TMPFS__ = new Map();

// Shared hand-off slot: the harness sets the single-slide document HTML here
// right before invoking html2pptx(); the playwright shim's goto() consumes it.
const __H2P_SHIM__ = { nextDocHTML: '' };
...
const BufferShim = {
  ...
  from: (x, enc) => {
    if (typeof x === 'string') {
      if (enc === 'base64') return _b64ToBytes(x);
      return new TextEncoder().encode(x); // utf8 / default
    }
...
const pathShim = {
  ...
  dirname: (p) => { const s = String(p).split('/'); s.pop(); return s.join('/') || '/'; },
  resolve: (...a) => pathShim.join(...a),
};

const fsShim = {
  // fixImageExtension reads file magic bytes off disk — there is no disk in the
  // browser, so make the sync reads throw; the engine's try/catch then returns
  // the path unchanged (which is exactly what we want for our __TMPFS__ paths).
  openSync: () => { throw new Error('ENOENT (browser fs shim)'); },
  readSync: () => { throw new Error('ENOENT (browser fs shim)'); },
  closeSync: () => {},
  copyFileSync: () => { throw new Error('ENOENT (browser fs shim)'); },
  existsSync: () => false,
  statSync: () => { throw new Error('ENOENT (browser fs shim)'); },
  readdirSync: () => [],
  promises: {
    readFile: async (p) => {
      if (__TMPFS__.has(p)) {
        const dataUrl = __TMPFS__.get(p);
        const b64 = dataUrl.split(',')[1] || '';
        return _b64ToBytes(b64);
      }
      throw new Error('ENOENT (browser fs shim): ' + p);
    },
  },
};
...
/* --------------------------------------------------------- http / https --- */
// Minimal node-style get() used only by the engine's downloadToBuffer(). Backed
// by fetch(); emits a single 'data' chunk then 'end'. Cross-origin images only
// succeed if the host sends permissive CORS headers (same constraint pptxgenjs
// has when it embeds images in the browser).
function _emitter() {
  const map = {};
  return {
    on(ev, fn) { (map[ev] = map[ev] || []).push(fn); return this; },
    once(ev, fn) { return this.on(ev, fn); },
    emit(ev, ...args) { (map[ev] || []).forEach((fn) => fn(...args)); },
    removeListener() { return this; },
  };
}
function httpGet(url, optsOrCb, maybeCb) {
  ...
  req.end = () => {};
  req.abort = () => {};
  fetch(url, { mode: 'cors' }).then(async (r) => {
    const res = _emitter();
    res.statusCode = r.status;
    res.headers = { location: r.headers.get('location') };
    res.resume = () => {};
    res.setEncoding = () => {};
    if (cb) cb(res);
    if (r.status >= 300 && r.status < 400) { res.emit('end'); return; }
    if (r.status < 200 || r.status >= 300) { res.emit('end'); return; }
    const ab = await r.arrayBuffer();
    res.emit('data', new Uint8Array(ab));
    res.emit('end');
  }).catch((e) => { req.emit('error', e); });
  return req;
}
const httpShim = { get: httpGet, request: httpGet };
...
async function _canvasToBytes(canvas) {
  let blob;
  if (canvas.convertToBlob) blob = await canvas.convertToBlob({ type: 'image/png' });
  else blob = await new Promise((res) => canvas.toBlob(res, 'image/png'));
  return new Uint8Array(await blob.arrayBuffer());
}
function _looksLikeSvg(bytes) {
  // sniff first few non-space bytes for '<'
...
async function _drawableFrom(input, rotate) {
  // SVG must go through an <img> element: Chrome's createImageBitmap() cannot
  // decode SVG blobs ("source image could not be decoded"). <img> handles SVG
  // data URLs fine and draws cleanly (untainted) to a canvas.
  if (typeof input === 'string' && /^data:image\/svg\+xml/i.test(input)) {
    const img = await _loadImageEl(input);
    return { img, width: img.naturalWidth || img.width || 1, height: img.naturalHeight || img.height || 1 };
  }
  if (input instanceof Uint8Array && _looksLikeSvg(input)) {
    const dataUrl = 'data:image/svg+xml;base64,' + _bytesToB64(input);
    const img = await _loadImageEl(dataUrl);
    return { img, width: img.naturalWidth || img.width || 1, height: img.naturalHeight || img.height || 1 };
  }
  // Raster images: fetch to a same-origin blob (avoids canvas taint) then decode.
  let blob;
  if (typeof input === 'string') {
    const r = await fetch(input, { mode: 'cors' });
    blob = await r.blob();
  } else {
    blob = new Blob([input]);
  }
  const opts = rotate ? { imageOrientation: 'from-image' } : undefined;
  const bmp = await createImageBitmap(blob, opts);
  return { img: bmp, width: bmp.width, height: bmp.height };
}
async function _sharpRender(state) {
  let canvas;
  if (state.input && state.input.create) {
    const c = state.input.create;
    canvas = _newCanvas(c.width, c.height);
    const ctx = canvas.getContext('2d');
    if (c.background) {
      const bg = c.background;
      ctx.fillStyle = `rgba(${bg.r || 0},${bg.g || 0},${bg.b || 0},${bg.alpha == null ? 1 : bg.alpha})`;
      ctx.fillRect(0, 0, canvas.width, canvas.height);
    }
  } else {
    const src = await _drawableFrom(state.input, state.rotate);
    canvas = _newCanvas(src.width, src.height);
    canvas.getContext('2d').drawImage(src.img, 0, 0);
  }
  for (const op of state.ops) {
    if (op.t === 'resize') {
      const nc = _newCanvas(op.w, op.h);
      nc.getContext('2d').drawImage(canvas, 0, 0, op.w, op.h);
      canvas = nc;
    } else if (op.t === 'extract') {
      const nc = _newCanvas(op.width, op.height);
      nc.getContext('2d').drawImage(
        canvas, op.left, op.top, op.width, op.height, 0, 0, op.width, op.height);
      canvas = nc;
    } else if (op.t === 'composite') {
      const ctx = canvas.getContext('2d');
      for (const item of op.arr) {
        const d = await _drawableFrom(item.input, false);
        ctx.drawImage(d.img, item.left || 0, item.top || 0);
      }
    }
  }
  return canvas;
}
function sharpShim(input, opts) {
  const state = { input, density: opts && opts.density, ops: [], rotate: false };
  const api = {
    metadata: async () => {
      const d = await _drawableFrom(state.input, false);
      return { width: d.width, height: d.height, format: 'png' };
    },
    rotate: () => { state.rotate = true; return api; },
    resize: (w, h, o) => { state.ops.push({ t: 'resize', w, h, fit: (o && o.fit) || 'fill' }); return api; },
    extract: ({ left, top, width, height }) => { state.ops.push({ t: 'extract', left, top, width, height }); return api; },
    composite: (arr) => { state.ops.push({ t: 'composite', arr }); return api; },
    png: () => api,
    jpeg: () => api,
    toBuffer: async () => _canvasToBytes(await _sharpRender(state)),
    toFile: async (p) => {
      const bytes = await _canvasToBytes(await _sharpRender(state));
      __TMPFS__.set(p, 'data:image/png;base64,' + _bytesToB64(bytes));
...
/* ----------------------------------------------------- playwright shim ---- */
// A hidden <iframe> stands in for a Playwright page. Because the iframe is
// same-origin (srcdoc), we can compile the engine's evaluate() callback inside
// the iframe realm via contentWindow.eval — so it measures the slide's real
// layout with the browser's actual CSS/layout engine.
function makePage() {
  ...
  const page = {
    _iframe: iframe,
    on(ev, cb) { if (ev === 'console') consoleCb = cb; return page; },
    route: async () => {},               // font interception not needed in iframe
    ...
    goto: async () => {
      const html = __H2P_SHIM__.nextDocHTML || '<!DOCTYPE html><html><body></body></html>';
      ...
      try {
        ...
        w.console.log = (...a) => { origLog(...a); if (consoleCb) consoleCb({ text: () => a.join(' ') }); };
      } catch (e) { /* ignore */ }
    },
    evaluate: async (fn, arg) => {
      const w = iframe.contentWindow;
      const src = '(' + fn.toString() + ')';
      const compiled = w.eval(src);                 // function bound to iframe realm
      let out = compiled(arg);
      if (out && typeof out.then === 'function') out = await out;
      if (out === undefined || out === null) return out;
      try { return JSON.parse(JSON.stringify(out)); } // copy into parent realm
      catch (e) { return out; }
    },
  };
  return page;
...
const chromiumShim = {
  launch: async () => {
    ...
    return {
      ...
      newContext: async () => ({ newPage: async () => { const p = makePage(); pages.push(p); return p; } }),
      close: async () => { pages.forEach((p) => { try { p._iframe.remove(); } catch (e) {} }); },
    };
  },
};
...
function shimRequire(name) {
  switch (name) {
    case 'playwright': return { chromium: chromiumShim };
    case 'playwright-core': return { chromium: chromiumShim };
    case 'path': return pathShim;
    case 'fs': return fsShim;
    case 'sharp': return sharpShim;
    case 'http': return httpShim;
    case 'https': return httpShim;
    case 'os': return { tmpdir: () => '/tmp' };
    default: throw new Error('shimRequire: unsupported module "' + name + '"');
  }
}
...
async function extractSlideData(page, slideDims) {
  return await page.evaluate((slideDims) => {
    ...
    // PPTX shape fills have no radial support, so we rasterize to PNG like the
    // linear path. We only need the center position + stops; size keywords
    // (circle/ellipse/closest-corner/130% 130%…) are approximated as a
    // farthest-corner radius, which is visually faithful for slide surfaces.
    const parseRadialGradientLayer = (layer) => {
      ...
      const headIsMeta = /\b(circle|ellipse|closest-side|closest-corner|farthest-side|farthest-corner|at)\b/i.test(head)
        || /^[\d.]+(px|%|em|rem|vw|vh)/i.test(head);
      if (headIsMeta) {
        stopParts = parts.slice(1);
        const atM = head.match(/\bat\s+(.+)$/i);
        if (atM) {
          const toks = atM[1].trim().split(/\s+/);
          const frac = (t) => {
            const s = t.toLowerCase();
            if (s === 'left' || s === 'top') return 0;
            if (s === 'right' || s === 'bottom') return 1;
            if (s === 'center') return 0.5;
            if (/%$/.test(s)) return Math.max(0, Math.min(1, parseFloat(s) / 100));
            return null;
          };
          if (toks[0] != null) { const v = frac(toks[0]); if (v != null) pos.x = v; }
          if (toks[1] != null) { const v = frac(toks[1]); if (v != null) pos.y = v; }
        }
      }
      const stops = stopParts.map((part, idx) => {
        const p = part.trim();
        ...
        return {
          color: cm[1],
          pos: posM ? Math.max(0, Math.min(1, parseFloat(posM[1]) / 100))
                    : (stopParts.length === 1 ? 0 : idx / (stopParts.length - 1))
        };
      }).filter(Boolean);
      return stops.length >= 2 ? { type: 'radial', pos, stops } : null;
    };

    // Dispatch a single background-image layer to the right gradient parser.
    const parseGradientLayer = (layer) => {
      if (/^\s*radial-gradient/i.test(layer)) return parseRadialGradientLayer(layer);
      return parseLinearGradientLayer(layer);
    };
    ...
    // Rasterize a CSS background-image (linear AND radial gradient layers) to a
    // PNG data URL. Layered under native text/shapes by the caller, so surfaces
    // stay pixel-faithful while text remains editable. `blurPx` (from a host
    // `filter: blur()`) softens decorative blobs/glows instead of emitting a
    // hard-edged shape.
    const renderGradientBackground = (bgImage, widthPx, heightPx, blurPx) => {
      const layers = splitCssTopLevel(bgImage).map(parseGradientLayer).filter(Boolean);
      ...
      const ctx = canvas.getContext('2d');
      if (blurPx && blurPx > 0) { try { ctx.filter = `blur(${blurPx}px)`; } catch (e) {} }
      const cx = canvas.width / 2;
      const cy = canvas.height / 2;
      for (const layer of layers.slice().reverse()) {
        let grad;
        if (layer.type === 'radial') {
          const rcx = (layer.pos ? layer.pos.x : 0.5) * canvas.width;
          const rcy = (layer.pos ? layer.pos.y : 0.5) * canvas.height;
          let r = 0;
          for (const [px, py] of [[0, 0], [canvas.width, 0], [0, canvas.height], [canvas.width, canvas.height]]) {
            const d = Math.hypot(px - rcx, py - rcy);
            if (d > r) r = d;
          }
          grad = ctx.createRadialGradient(rcx, rcy, 0, rcx, rcy, Math.max(1, r));
        } else {
          ...
          grad = ctx.createLinearGradient(cx - vx * len / 2, cy - vy * len / 2, cx + vx * len / 2, cy + vy * len / 2);
        }
        for (const stop of layer.stops) {
          try { grad.addColorStop(Math.max(0, Math.min(1, stop.pos)), stop.color); } catch (e) {}
        }
        ctx.fillStyle = grad;
        ctx.fillRect(0, 0, canvas.width, canvas.height);
      }
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      // Icon web-components (iconify-icon, <svg>-in-shadow custom elements) render
      // their glyph into shadow DOM, which the light-DOM walk never enters — so
      // icons silently vanished. Capture the shadow <svg> as an image at the
      // host's box, baking `currentColor` to the host's computed color.
      if (el.shadowRoot && typeof el.shadowRoot.querySelector === 'function' &&
        ...
        if (rect.width > 0 && rect.height > 0) {
          try {
            ...
            const svgEl = el.shadowRoot.querySelector('svg').cloneNode(true);
            ...
            bake(svgEl);
            svgEl.querySelectorAll('*').forEach(bake);
            // iconify glyphs default to fill:currentColor with no explicit fill attr.
      ...
      if (isContainer) {
        ...
        if (!isTextClippedBackground && bgImage && bgImage !== 'none') {
          ...
          // A `filter: blur(Npx)` on the host (decorative blobs/glows) is baked
          // into the raster so it reads as a soft glow rather than a hard shape.
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    try {
      ...
      // Icon web-components (iconify-icon) fetch glyph data asynchronously from a
      // CDN and render into shadow DOM. Wait until they've rendered, but NEVER
      // block conversion on a slow/unreachable CDN: cap the wait, then report a
      // clear, non-fatal warning for any icons that didn't make it.
      try {
        iconStatus = await page.evaluate(async (timeouts) => {
          const icons = Array.from(document.querySelectorAll('iconify-icon'));
          if (!icons.length) return { total: 0, rendered: 0, scriptLoaded: true, timedOut: false };
          const isRendered = (el) => !!(el.shadowRoot && el.shadowRoot.querySelector('svg'));
          // First wait (briefly) for the Iconify script itself to define the element.
          const scriptDeadline = Date.now() + timeouts.script;
          while (!(window.customElements && window.customElements.get('iconify-icon')) && Date.now() < scriptDeadline) {
            await new Promise((r) => setTimeout(r, 100));
          }
          const scriptLoaded = !!(window.customElements && window.customElements.get('iconify-icon'));
          // Then wait for glyph data to render — but only if the script loaded.
          if (scriptLoaded) {
            const deadline = Date.now() + timeouts.data;
            while (Date.now() < deadline && icons.some((el) => !isRendered(el))) {
              await new Promise((r) => setTimeout(r, 100));
            }
          }
          const rendered = icons.filter(isRendered).length;
          return { total: icons.length, rendered, scriptLoaded, timedOut: rendered < icons.length };
        }, { script: 2000, data: 4000 });
      } catch (e) {
        iconStatus = { error: e.message };
      }
    ...
    if (iconStatus) {
      if (iconStatus.error) {
        allWarnings.push(`⚠ ICONS: readiness check failed (${iconStatus.error}) — continued without icons.`);
      } else if (iconStatus.total > 0 && iconStatus.rendered < iconStatus.total) {
        const missing = iconStatus.total - iconStatus.rendered;
        const reason = iconStatus.scriptLoaded
          ? 'icon data timed out (CDN slow/unreachable: api.iconify.design)'
          : 'the Iconify script did not load (CDN unreachable: code.iconify.design)';
        allWarnings.push(`⚠ ICONS: ${missing}/${iconStatus.total} icon(s) skipped — ${reason}. Conversion continued; re-run when the connection is stable to embed them.`);
      }
    }
...
module.exports = html2pptx;
  })(module, exports, shimRequire, processShim, BufferShim, '/', '/html2pptx.js');
...
function _patchSlideAddImage(slide) {
  const orig = slide.addImage.bind(slide);
  slide.addImage = (opts) => {
    if (opts && opts.path && __TMPFS__.has(opts.path)) {
      const o = Object.assign({}, opts, { data: __TMPFS__.get(opts.path) });
      delete o.path;
      return orig(o);
    }
    return orig(opts);
